...
public:
    ...
    bool hook() { return hook_to_notification_point(HT_IDP, IDP_Callback, this); }
    bool unhook() { return unhook_from_notification_point(HT_IDP, IDP_Callback, this); }
    /* Hook functions to override in Python */
    
    virtual int add_cref(ea_t from, ea_t to, cref_t type)
    {
        return 0;
    }
};
...
int idaapi IDP_Callback(void *ud, int notification_code, va_list va)
{
    class IDP_Hooks *proxy = (class IDP_Hooks *)ud;

    try {
        switch (notification_code) {
        case processor_t::add_cref: {
            // MUST assign to temporaries prior to passing to the proxy, or somehow the values get dorked
            ea_t from = va_arg(va, ea_t);
            ea_t to = va_arg(va, ea_t);
            cref_t type = va_arg(va, cref_t);
            return proxy->add_cref(from, to, type);
        } break;
        }
